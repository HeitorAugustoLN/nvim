          ...
          let
            args =
              ...
              in
              {
                ...
                extraBuilderArgs = {
                  # Disabled on dev because it fails to build on CI because of the impure path
                  doInstallCheck = !dev;

                  # Set HOME as neovim creates files during startup
                  # Check stderr since neovim doesn't exit with non-zero on config errors
                  installCheckPhase = ''
